...
import {PacksCardsAPI} from '../b3-dal/api';
import {ThunkDispatch, ThunkAction} from 'redux-thunk';
import {CardsPackType, PackType, PropsUpdatePackType} from '../b1-ui/common/utils/types';
...
export const UpdatePackCards = (data:PropsUpdatePackType): ThunkType => {
    return async (dispatch: ThunkDispatch<AppStateType, unknown, ActionType>, getState: () => AppStateType) => {
        let token = restoreStateLocalStorage('authToken', '')

        try {
            let res = await PacksCardsAPI.updatePackWithCards(data, token)
            saveStateToLocalStorage(res.data.token, 'authToken')
            dispatch(GetPacksCards())
        } catch (e) {
            let errorText = e.response.data.error;
